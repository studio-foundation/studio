...
**Trigger** — A `.trigger.yaml` in `.studio/triggers/`: an inbound webhook that verifies an HMAC signature, matches the payload against `when:` conditions (stage-condition syntax, over `payload.<path>`), maps it into pipeline input, and launches a run. Served by `studio api start` at `POST /api/triggers/<name>/webhook`. It exists because MCP and tools are outbound-only; everything a trigger says *back* to the external system is an ordinary tool call, and `on_failure` is a shell command receiving its values through the environment (`STUDIO_RUN_ID`, `STUDIO_META`, …), never interpolated into the command string. The kernel names no product: which events count, which field becomes which input, what failure does are all the trigger file's opinions, so a new one is a marketplace package, not a Studio release. This replaced `.integration.yaml` and `api/src/integrations/` wholesale ([ADR 0004](docs/adr/0004-triggers-over-integrations.md)). See CONCEPTS.md.

**Packaging types** — A marketplace publishes exactly two: `template` (starts a project — `studio init --template X`) and `plugin` (adds to one — `studio plugin add X`). Tools, agents, skills, triggers, pipelines, contracts and inputs are content kinds *inside* a plugin, declared in `provides` and dispatched on install to their `.studio/` subdirectory by filename suffix. Filenames are kept as published — the agent and skill loaders resolve by filename. The lockfile records the paths written (`files`), so `remove` and `audit` act on what was installed instead of re-deriving a path from a type; entries without `files` fall back to the pre-migration derivation ([legacy-paths.ts](cli/src/registry/legacy-paths.ts)). See [ADR 0002](docs/adr/0002-packaging-model.md).
...
That is the whole list. `git`, `search`, `web-search` and every vendor trigger are
