...
**Fan-out (map) stages** — A `map:` entry runs a sub-pipeline once per item of a list and collects the structured outputs. It replaces the "shell `studio run` per item + scrape the run log" glue: child runs are spawned in-process via the run spawner, each returning its last-stage output directly. Config: `over` (context path to the list), `pipeline` (sub-pipeline per item), `input`/`as` (per-item input), `concurrency` (default 1), `on_item_failure` (`fail-fast` default, or `collect-all`), `resume` (default false — per-item resume: skip items already completed in a prior run, keyed on the item **input** not its index, optionally widened with `resume: { key_on: [input, agent, contract] }` (parsed-document hash of agents, skills, invariants, contracts; comments and key order ignored); failures are never cached; cache lives at `.studio/runs/map-cache/`), `batch` (default false — see below). Output is `{ total, succeeded, failed, resumed, outputs, results }`. See CONCEPTS.md.
...
**Token usage** — Every provider reports what a call spent (`TokenUsage` in [contracts/src/usage.ts](contracts/src/usage.ts)): `prompt_tokens`, `completion_tokens`, `total_tokens`, plus `cached_input_tokens` / `cache_creation_tokens` and a `by_model` split. The four count fields are disjoint because each is billed at its own rate; providers normalize to that definition (Anthropic excludes cache counts from its input total, OpenAI includes them). The runner sums it across the turns of one agent run, the engine across the attempts of a RALPH loop — a retried stage reports the retries too — and a `call`/`map` stage carries the roll-up of the child runs it spawned. It lands on the stage, on the run, and on each event in `.studio/runs/<run>.jsonl`, so pricing a run is a `jq` pass rather than a correlation of provider session files against stage timestamps. A spawned child also reports its per-stage breakdown across the spawner boundary (`SpawnResult.stages`, and `ChildRunError` when it failed), so a `map` item says how many attempts bought its number instead of pricing as one call. `studio status` aggregates it per stage and per model. See CLI.md.

**Prompt caching (`prompt_cache`)** — A prompt cache write is billed above the plain input rate and only earns that back when a later call reads the same prefix, so a call with no successor costs *more* cached than uncached (a measured run: 4.5M cache-write tokens against 1.05M reads, +35% on the bill). The provider therefore never caches speculatively — it marks the prefix only when the caller asks, and the caller is the runner, which is the only party that knows whether a second turn is coming ([prompt-cache.ts](runner/src/prompt-cache.ts)). Agents set `prompt_cache: auto | on | off`; `auto` caches iff the stage's *contract* obliges it to call tools (`tool_calls.minimum`/`required_tools`/`required_tool_groups`) — that is what predicts a multi-turn loop, whereas tools merely being available is the single-shot case that was overpaying. Decided once per agent run and held for every turn. 5m TTL only: the 1h TTL doubles the write premium and needs three reads, not two, to break even. See CONCEPTS.md.
...
**Disabling interactive prompts (`interactive: false`)** — `ask` and `approval:` both activate purely on TTY detection, which any pipeline YAML can trigger with no project-level consent. `interactive: false` in `.studio/config.yaml` forces both down their existing non-interactive path regardless of whether a real TTY is attached — same gate `cli/src/commands/run.ts` already applies (`process.stdin.isTTY && process.stdout.isTTY`), with `config.interactive !== false` added to the same condition. No pipeline YAML can override it: the key lives outside any pipeline file. Default `true`.
...
**Packaging types** — A marketplace publishes exactly two: `template` (starts a project — `studio init --template X`) and `plugin` (adds to one — `studio plugin add X`). Tools, agents, skills, pipelines, contracts and inputs are content kinds *inside* a plugin, declared in `provides` and dispatched on install to their `.studio/` subdirectory by filename suffix. Filenames are kept as published — the agent and skill loaders resolve by filename. The lockfile records the paths written (`files`), so `remove` and `audit` act on what was installed instead of re-deriving a path from a type; entries without `files` fall back to the pre-migration derivation ([legacy-paths.ts](cli/src/registry/legacy-paths.ts)). See [ADR 0002](docs/adr/0002-packaging-model.md).

**Marketplaces** — A marketplace is a git repo with an `index.json` at its root. `studio-community` is the default and needs no registration; `studio marketplace add <url>` registers another in `~/.studio/marketplaces.json` — per machine, never per project, so a checkout cannot redirect its own installs. GitHub marketplaces are read over raw HTTP, anything else is shallow-cloned, so a self-hosted GitLab works unmodified. An entry's `source` is `local` (the marketplace repo, reviewed as a diff) or `git` (someone else's repo, where review only ever sees the entry — so the fetch checks the pinned `sha` against its `ref`, asserts a LICENSE matching `license`, and asserts `provides` against the payload in both directions; any mismatch fails the install, [verify.ts](cli/src/registry/verify.ts)). A name is unique per marketplace, not globally: an unqualified name found in several is refused with the qualified forms, never resolved by registration order. See [ADR 0001](docs/adr/0001-distribution-model.md), CLI.md.
...
**Run retention (`runs.retention.max_age_days`)** — Deletes `.studio/runs/*.jsonl` logs, their anonymization keymaps and `runs.db` rows older than N days at the end of `studio run`; `studio runs prune [--keep-last N] [--max-age 30d] [--keep-status s] [--dry-run]` is the manual form ([runs-retention.ts](cli/src/runs-retention.ts)). Keymap deletion reuses the engine's `purgeKeymaps`, row deletion `SQLiteRunStore.purgePipelineRuns`; map-cache is left to `studio cache clean`. See CLI.md, CONCEPTS.md.

**Seed cache** — `cli/templates/seed/` is a pre-fetched snapshot of the official marketplace, bundled through the same `BUNDLED_ASSETS` step as the templates. It mirrors the marketplace layout verbatim (`index.json` plus each package's `source.path` tree), so every lookup is a path lookup and no kernel code names a package. `studio init` and `studio registry install` fall back to it when the network is unreachable, which is what lets `git` and `search` leave the kernel without making a fresh install require connectivity (INV-11). A seeded package is an ordinary package — removable, overridable, superseded by the live registry the moment one answers. The seeded index is deliberately never written to the 24-hour cache, or the next online run would skip its sync. Refresh with `pnpm seed:refresh` ([scripts/refresh-seed.mjs](scripts/refresh-seed.mjs)), which needs `GITHUB_TOKEN` (the contents API rate-limits unauthenticated calls with a 403) or a local checkout as its argument. A weekly workflow, [seed-drift.yml](.github/workflows/seed-drift.yml), runs it with the CI token and opens the PR itself; `gh workflow run seed-drift.yml` triggers it now.
...
**Preflight (`studio doctor`)** — One command that aggregates every startup check `studio run` performs separately (Studio version, config contract, required binaries, declared script interpreters) plus two checks `run` doesn't have at that breadth: an env-var check (a `${VAR}` with nothing behind it resolves to an empty string, so the key passes the contract while carrying no value) and contract coverage across every pipeline in `.studio/pipelines/`, not only the one being run. Green/red list, exit 1 on any failure, warnings don't fail. Checks live in `cli/src/preflight.ts`, printing in `cli/src/commands/doctor.ts`. See CLI.md.
...
Distribution: GitHub Releases (+ `install.sh` for `curl | sh`, `install.ps1` for `irm | iex` on Windows) and npm, where per-platform packages (`@studio-foundation/cli-<platform>`) are optional dependencies of `@studio-foundation/cli` and `cli/bin/studio.mjs` runs whichever one installed, falling back to the JS build. `studio upgrade` (`cli/src/upgrade.ts`) is the update path for the binary channel only — it detects an npm-owned install from `process.execPath` and refuses, since overwriting a file npm tracks breaks the next `npm i -g`. See CLI.md.
