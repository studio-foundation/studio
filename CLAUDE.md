...
Distribution: GitHub Releases (+ `install.sh` for `curl | sh`, `install.ps1` for `irm | iex` on Windows) and npm, where per-platform packages (`@studio-foundation/cli-<platform>`) are optional dependencies of `@studio-foundation/cli` and `cli/bin/studio.mjs` runs whichever one installed, falling back to the JS build. See CLI.md.
