...
**Run retention (`runs.retention.max_age_days`)** — Deletes `.studio/runs/*.jsonl` logs and their anonymization keymaps older than N days at the end of `studio run`; `studio runs prune [--keep-last N] [--max-age 30d] [--keep-status s] [--dry-run]` is the manual form ([runs-retention.ts](cli/src/runs-retention.ts)). Keymap deletion reuses the engine's `purgeKeymaps`. See CLI.md, CONCEPTS.md.
