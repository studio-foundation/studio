...
export interface RepoResolveOptions {
  /** Explicit local path (--repo flag equivalent). Skips URL resolution. */
  repoPathOverride?: string;
  /** Git URL to clone. Requires rawProjectsDir or STUDIO_PROJECTS_DIR env var. */
  repoUrl?: string;
  /** Raw projects_dir from config (may contain ~). Falls back to STUDIO_PROJECTS_DIR env var. */
  rawProjectsDir?: string;
  /** Pipeline name — used as directory prefix when cloning. */
  pipelineName: string;
  /** Optional branch to checkout when cloning. */
  branch?: string;
}
