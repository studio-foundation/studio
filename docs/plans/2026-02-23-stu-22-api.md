# @studio-foundation/api Phase 1 Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Créer le package `@studio-foundation/api` avec Fastify — REST API pour lancer et consulter des runs Studio.

**Architecture:** Package standalone (`api/`) qui dépend de `@studio-foundation/engine` + `@studio-foundation/runner` + `@studio-foundation/contracts`. CLI et API sont deux interfaces indépendantes sur le même engine. `buildServer(deps)` prend store + launcher en injection de dépendance. `RunStore` (interface) partout — jamais `SQLiteRunStore` directement sauf dans `bootstrap.ts`.
...
```bash
cd /path/to/Studio && pnpm --filter @studio-foundation/engine build
```
...
### Task 3: Scaffold @studio-foundation/api package
...
pnpm install
```
Expected: `@studio-foundation/api` reconnu dans le workspace, fastify installé.

**Step 5: Commit**

```bash
git add api/package.json api/tsconfig.json pnpm-workspace.yaml pnpm-lock.yaml api/src/.gitkeep
git commit -m "feat(api): scaffold @studio-foundation/api package"
```
...
import { resolve } from 'node:path';
import type { PipelineEngine } from '@studio-foundation/engine';
...
export async function bootstrap(cwd: string = process.cwd()): Promise<BootstrapResult> {
  ...
  for (const manifest of pluginManifests) {
    for (const [serverName, serverDef] of Object.entries(manifest.mcpServers)) {
      const { MCPClient: MCPClientClass } = await import('@studio-foundation/runner');
...
```typescript
#!/usr/bin/env node
// Standalone entrypoint for @studio-foundation/api
...
- Modify: `cli/package.json` (ajouter dépendance `@studio-foundation/api`)
...
**Step 2: Ajouter `@studio-foundation/api` aux dépendances de `cli/package.json`**
...
```typescript
import { bootstrap, buildServer } from '@studio-foundation/api';
...
**Step 4: Exporter depuis `@studio-foundation/api/src/index.ts`**
...
```bash
pnpm --filter @studio-foundation/cli typecheck && pnpm --filter @studio-foundation/api typecheck
```
...
```bash
git add -A
git commit -m "feat(api): @studio-foundation/api Phase 1 complete — Fastify REST API"
```
...
| `cli/package.json` | Modify — ajouter `@studio-foundation/api` |
