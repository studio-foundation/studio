...
| AC4 — shared keymap under partial scoping | same PII value in a scoped AND an unscoped field → tokenized in scoped, cleartext in unscoped, round-trips via `deanonymize`, no reconstruction inconsistency | `runner/tests/anonymize-fields.test.ts` |
