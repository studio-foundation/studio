...
cp .studio/config.example.yaml .studio/config.yaml   # fill in what it asks for
studio doctor
```

`studio doctor` answers a single question — **can this machine run this project?** — by checking the installed Studio against the project's `studio_version`, `config.yaml` against the `config.example.yaml` contract, every required binary against PATH, and every `${VAR}` for a non-empty value. Exit code is `1` on any failure, so it works as a CI or bootstrap gate. See [CLI.md](./CLI.md#preflight--studio-doctor).

Projects generated by `studio init` carry this as `ONBOARDING.md` at their root.
...
Six packages, one monorepo. Each fits in a single context window. Each is testable in isolation. ralph doesn't know runner. runner doesn't know engine. contracts is a leaf. The dependency graph is the architecture; the architecture is the project's politics.
...
[studio-community](https://github.com/studio-foundation/studio-community) is the shared registry for tools, templates, pipelines, agents, plugins, and skills. Open publish, no review gate.
...
**Studio is in beta.** The core works (RALPH loop, pipeline engine, groups, hooks, tools, multi-provider, CLI, API) but expect rough edges, breaking changes, and missing pieces. The architecture is stable; the surface is still moving. Current version is whatever `npm view @studio-foundation/cli version` returns.
